...
Usage: $(basename "$0") [--build | --serve]
  ...
  --build  (default) Build the SPA in a one-shot docker container and write
           the resulting dist/ to /frontend_app_dev/dist on the host. Host
...
sudo rm -rf /frontend_app_${SITE}/dist
...
# step onward, so `pnpm run build:dev` actually re-executes every invocation
