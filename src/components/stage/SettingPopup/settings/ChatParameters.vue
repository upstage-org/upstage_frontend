...
export default {
  components: { HorizontalField, Field, SaveButton, AppSwitch, DownloadButton },
  emits: ["close"],
  setup: (props, { emit }) => {
    const form = reactive({});
...
<template>
  ...
  <div class="card-content">
    <div v-if="currentTab === 'nickname'" class="content">
      ...
      <SaveButton :loading="loading" @click="saveNickname" />
    </div>
    <div v-else-if="currentTab === 'params'" class="content">
      ...
      <HorizontalField title="Size (px)">
        <Field
          :model-value="parameters.fontSize?.slice(0, -2)"
          type="number"
          @update:model-value="changeFontSize"
        />
      </HorizontalField>

      <SaveButton @click="saveParameters" />
    </div>
    <div v-else class="content">
        ...
        <div class="field-body">
          <div class="field is-narrow">
            <a-tooltip :title="downloadOptions.audienceChat ? 'On' : 'Off'">
              <AppSwitch v-model="downloadOptions.audienceChat" />
            </a-tooltip>
          </div>
        </div>
      </div>
        ...
        <div class="field-body">
          <div class="field is-narrow">
            <a-tooltip :title="downloadOptions.playerChat ? 'On' : 'Off'">
              <AppSwitch v-model="downloadOptions.playerChat" />
            </a-tooltip>
          </div>
        </div>
      </div>
      ...
      />
      <DownloadButton v-else disabled />
    </div>
  </div>
</template>
