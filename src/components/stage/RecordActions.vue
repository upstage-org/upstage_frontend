...
export default {
  ...
  setup: (props) => {
    ...
    const deleteRecording = async (complete) => {
      ...
      // Parent passes `stage` for in-place mutation here so the UI flips
      // immediately; the GraphQL refresh() below is the canonical source of
      // truth, this just shortens the UX gap. Suppress vue/no-mutating-props
      // rather than rewire the parent.
    ...
    const saveRecording = async () => {
      await saveMutation(() => {
        ...
        // See deleteRecording above for rationale.
        // eslint-disable-next-line vue/no-mutating-props
        Object.assign(props.stage, {
          activeRecording: null,
        });
...
<template>
  ...
  <div v-else-if="stage.activeRecording" class="field has-addons">
    <p class="control">
      <a-tooltip title="Stop recording">
        <button class="button is-small is-light is-danger" @click="saveRecording">
    ...
    <p class="control">
      <CustomConfirm :loading="deleting" @confirm="(complete) => deleteRecording(complete)">
        <template #trigger>
  ...
  <template v-else>
    <CustomConfirm :loading="loading" @confirm="(complete) => startRecording(complete)">
