...
export default {
  setup() {
    ...
    const isAdmin = computed(
      () =>
        whoami.value &&
        [String(configs.ROLES.ADMIN), String(configs.ROLES.SUPER_ADMIN)].includes(
          String(whoami.value.role),
        ),
    );
    return () =>
      h(
                  ...
                  ? [
                      {
                        children: h(
                          SubMenu,
                          {
                            ...
                            style: {
                              margin: 4,
                              background: router.currentRoute.value.path.startsWith("/admin")
